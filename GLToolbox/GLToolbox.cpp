...
std::array<float, 2> randPair(float min, float max) {
	return { myrand(min, max), myrand(min, max) };
}
std::array<float, 3> randCoord(float min, float max) {
	return { myrand(min, max), myrand(min, max), myrand(min, max) };
}
std::array<float, 3> translate(std::array<float, 3> coordinates, std::array<float, 3> dCoordinates) {
	return { coordinates[0] + dCoordinates[0], coordinates[1] + dCoordinates[1], coordinates[2] + dCoordinates[2] };
}
std::array<float, 3> rotateX(std::array<float, 3> coordinates, float theta) {
	float x = coordinates[0];
	float y = coordinates[1];
	float z = coordinates[2];
	return { x, y*cos(theta) - z * sin(theta), y*sin(theta) + z * cos(theta) };
}
std::array<float, 3> rotateY(std::array<float, 3> coordinates, float theta) {
	float x = coordinates[0];
	float y = coordinates[1];
	float z = coordinates[2];
	return { x*cos(theta) + z * sin(theta), y, -x * sin(theta) + z * cos(theta) };
}
std::array<float, 3> rotateZ(std::array<float, 3> coordinates, float theta) {
	float x = coordinates[0];
	float y = coordinates[1];
	float z = coordinates[2];
	return { x*cos(theta) - y * sin(theta), x*sin(theta) + y * cos(theta), z };
}
std::array<float, 3> scale(std::array<float, 3> coordinates, std::array<float, 3> diff) {
	float x = coordinates[0];
	float y = coordinates[1];
	float z = coordinates[2];
	float dx = diff[0];
...
std::array<float, 2> projectPoint(std::array<float, 3> coordinates, float d) {
...
const  int BOTTOM = 4;
const  int TOP = 8;
int outcode(std::array<float, 2> coordinates) {
	float x = coordinates[0];
	float y = coordinates[1];
	int b0, b1, b2, b3;
	b0 = y > 1 ? 8 : 0;
...
bool acceptSegment(std::array<float, 2> seg0, std::array<float, 2> seg1) {
	int code0 = outcode(seg0);
	int code1 = outcode(seg1);
	return !(code0 | code1);
}
bool discardSegment(std::array< float, 2> seg0, std::array<float, 2> seg1) {
	int outcode0 = outcode(seg0);
	int outcode1 = outcode(seg1);
	return outcode0 & outcode1;
}
bool clipSegemnt(std::array< float, 2> seg0, std::array<float, 2> seg1) {
	int outcode0 = outcode(seg0);
	int outcode1 = outcode(seg1);
...
float dotProduct(std::array<float, 3> vectorA, std::array<float, 3> vectorB) {
	float count = 0;
...
float diffuseTerm(std::array<float, 3> L, std::array<float, 3> N) {
	return dotProduct(L, N);
}
std::array<float, 3> idealReflection(std::array<float, 3> L, std::array<float, 3> N) {
...
float specularTerm(std::array<float, 3> V, std::array<float, 3> R, float P) {
...
int main() {
	...
	std::cout << std::endl << "Rotating testCoordinates0 on X, Y, and Z by 90deg. " << std::endl;
	printArray(rotateX(testCoordinates0, 90));
	printArray(rotateY(testCoordinates0, 90));
	printArray(rotateZ(testCoordinates0, 90));
	std::cout << std::endl << "Scaling testCoordinates0 by testCoordinates1. " << std::endl;
	printArray(scale(testCoordinates0, testCoordinates1));
	float randProject = myrand(0, 1);
	std::cout << std::endl << "Projecting testCoordinates0 by " << randProject << std::endl;
	printPair(projectPoint(testCoordinates0, randProject));
	std::array<float, 2> randPair0 = randPair(3, 3);
	std::cout << std::endl << "Getting outcode for pair "; printPair(randPair0);
	std::cout << outcode(randPair0) << std::endl;;
	std::array<float, 2> randPair1 = randPair(-2, 4);
	std::array<float, 2> randPair2 = randPair(-2, 4);
	std::cout << std::endl << "acceptSegment discardSegment and clipSegemnt for pair "; printPair(randPair1); std::cout << ","; printPair(randPair2);
	...
	std::cout << std::endl << "specularTerm with testCoordinates0 and testCoordinates1, value P is " << randomP << std::endl;
